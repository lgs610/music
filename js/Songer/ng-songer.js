var SongerApp = angular.module('SongerApp',['ngRoute']);


SongerApp.controller('Controller_songer',function($scope,$http){
	...
	$.extend($scope.setting,$scope.getSetting());
		...
		$http.get("data/songer/songername.json")
		...
		$scope.DataSonger;
		$http.get("data/songer/songer.json")
			.then(function(response){
				$scope.DataSonger=response.data;
			});	
		//按钮事件
