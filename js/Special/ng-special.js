...
MyMusicApp.controller('Controller_mymusic',function($scope,$http){
	...
	$.extend($scope.setting,$scope.getSetting());
		...
		$http.get("data/special/special.json")
	...
	//获取分类a标签
//	$scope.Kind_a=function(event){
//		if($(".songMenuChoose").text()=="全部歌单"){
//			//清空，然后加入分类选项
//			$(".songMenuChoose").html("");
//		}
//			$scope.newBtn = $('<div class="delBtn">×</div>');
//			$scope.newBtn.click(function(){//添加消除函数
//				this.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode);
//				if($(".songMenuChoose").html()==""){
//					$(".songMenuChoose").html("全部歌单");
//				}
//				$scope.language.false;
//			});
//			$scope.newPart = $('<div class="kind_choose_part">'+event.target.innerText+'</div>');
//			$scope.newKind = $('<div class="kind_choose_kind"></div>');
//			$scope.newPart.append($scope.newBtn);
//			$scope.newKind.append($scope.newPart);
//			$(".songMenuChoose").append($scope.newKind);
//			
//			$scope.language=true;
//	}

	$scope.Kind_l=function(event){
		if($(".songMenuChoose").css("display")=="block"){//清除全部歌单文本
			$(".songMenuChoose").css({display:"none"});
		}
		if($("#songLanguage").css("display")=="block"){//已经有了，替换文本
			$("#songlanguage").html(event.target.innerHTML);
		}else{
			$("#songlanguage").html(event.target.innerHTML);//先替换文本，再显示文本
			$("#songLanguage").css({display:"block"});
		}
		//过滤数据event.target.innerHTML
	}
	
	$scope.Kind_G=function(event){
		if($(".songMenuChoose").css("display")=="block"){//清除全部歌单文本
			$(".songMenuChoose").css({display:"none"});
		}
		if($("#songGenre").css("display")=="block"){//已经有了，替换文本
			$("#songgenre").html(event.target.innerHTML);
		}else{
			$("#songgenre").html(event.target.innerHTML);//先替换文本，再显示文本
			$("#songGenre").css({display:"block"});
		}
		//过滤数据event.target.innerHTML
	}
	$scope.Kind_T=function(event){
		if($(".songMenuChoose").css("display")=="block"){//清除全部歌单文本
			$(".songMenuChoose").css({display:"none"});
		}
		if($("#songTemme").css("display")=="block"){//已经有了，替换文本
			$("#songtemme").html(event.target.innerHTML);
		}else{
			$("#songtemme").html(event.target.innerHTML);//先替换文本，再显示文本
			$("#songTemme").css({display:"block"});
		}
		//过滤数据event.target.innerHTML
	}
	$scope.Kind_M=function(event){
		if($(".songMenuChoose").css("display")=="block"){//清除全部歌单文本
			$(".songMenuChoose").css({display:"none"});
		}
		if($("#songMood").css("display")=="block"){//已经有了，替换文本
			$("#songmood").html(event.target.innerHTML);
		}else{
			$("#songmood").html(event.target.innerHTML);//先替换文本，再显示文本
			$("#songMood").css({display:"block"});
		}
		//过滤数据event.target.innerHTML
	}
	$scope.Kind_S=function(event){
		if($(".songMenuChoose").css("display")=="block"){//清除全部歌单文本
			$(".songMenuChoose").css({display:"none"});
		}
		if($("#songScene").css("display")=="block"){//已经有了，替换文本
			$("#songscene").html(event.target.innerHTML);
		}else{
			$("#songscene").html(event.target.innerHTML);//先替换文本，再显示文本
			$("#songScene").css({display:"block"});
		}
		//过滤数据event.target.innerHTML
	}
	
	$scope.Kind_deleteL=function(event){
		$("#songLanguage").css({display:"none"});
		if(($("#songLanguage").css("display")=="none")&&($("#songGenre").css("display")=="none")&&($("#songTemme").css("display")=="none")&&($("#songMood").css("display")=="none")&&($("#songScene").css("display")=="none")){
			$(".songMenuChoose").css({display:"block"});
		}
	}
	$scope.Kind_deleteG=function(event){
		$("#songGenre").css({display:"none"});
		if(($("#songLanguage").css("display")=="none")&&($("#songGenre").css("display")=="none")&&($("#songTemme").css("display")=="none")&&($("#songMood").css("display")=="none")&&($("#songScene").css("display")=="none")){
			$(".songMenuChoose").css({display:"block"});
		}
	}
	$scope.Kind_deleteT=function(event){
		$("#songTemme").css({display:"none"});
		if(($("#songLanguage").css("display")=="none")&&($("#songGenre").css("display")=="none")&&($("#songTemme").css("display")=="none")&&($("#songMood").css("display")=="none")&&($("#songScene").css("display")=="none")){
			$(".songMenuChoose").css({display:"block"});
		}
	}
	$scope.Kind_deleteM=function(event){
		$("#songMood").css({display:"none"});
		if(($("#songLanguage").css("display")=="none")&&($("#songGenre").css("display")=="none")&&($("#songTemme").css("display")=="none")&&($("#songMood").css("display")=="none")&&($("#songScene").css("display")=="none")){
			$(".songMenuChoose").css({display:"block"});
		}
	}
	$scope.Kind_deleteS=function(event){
		$("#songScene").css({display:"none"});
		if(($("#songLanguage").css("display")=="none")&&($("#songGenre").css("display")=="none")&&($("#songTemme").css("display")=="none")&&($("#songMood").css("display")=="none")&&($("#songScene").css("display")=="none")){
			$(".songMenuChoose").css({display:"block"});
		}
	}
